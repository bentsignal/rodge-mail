...
export const Route = createFileRoute("/_authed/spam/messages/$messageId")({
  ...
  component: SpamMessagePage,
});

function SpamMessagePage() {
  const messageId = Route.useLoaderData({ select: (data) => data.messageId });
  const threadId = Route.useLoaderData({ select: (data) => data.threadId });
